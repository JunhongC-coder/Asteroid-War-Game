...
Sprite = function () {
  ...
  //update the objects position according to the board
  ...
  //display object'transform
  this.configureTransform = function () {
    if (!this.visible) return;

    var rad = (this.rot * Math.PI)/180;

    this.context.translate(this.x, this.y); //objects' position
    this.context.rotate(rad);  //objects' rotation speed
    this.context.scale(this.scale, this.scale);//size of objects
  };
  //draw object
  this.draw = function () {
    if (!this.visible) return;
  ...
  //objects position according to the baord
...
Ship = function () {
  ...
  //console.log(this.children.exhaust);
  ...
  //contorl the movement of spaceship
...
$(function () {
  ...
  var ship = new Ship();
  //sip's position at the beginning of the game
