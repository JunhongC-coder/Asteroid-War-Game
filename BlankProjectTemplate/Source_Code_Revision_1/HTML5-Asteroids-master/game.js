/**
 * @file game.js is the JavaScript file for one player mode functionality
  ...
  * @type {Float} */
  ...
  this.move = function (delta) {
    ...
    if ($.isFunction(this.preMove)) {
      ...
      //delta: time
    this.vel.x += this.acc.x * delta;//v=at
    this.vel.y += this.acc.y * delta;
    this.x += this.vel.x * delta;//s=vt
  ...
  /**Updates the sprite's position according to the game interface*/
  ...
  this.draw = function () {
    ...
    this.context.beginPath();//create a path

    this.context.moveTo(this.points[0], this.points[1]);
    //move the printer to the new position (points[0], points[1])
    for (var i = 1; i < this.points.length/2; i++) {
      var xi = i*2;
      var yi = xi + 1;
      this.context.lineTo(this.points[xi], this.points[yi]);
      //draw a line to the point(points[xi], points[yi])
    }

    this.context.closePath();
    this.context.stroke();//draw paths
  };
  /**
  ...
  /**
   * Check whether the collision is over.
   * @returns Returns true if the collision is over, otherwise returns false
   */
   ...
   * @returns Returns x coordinate of the sprite
   */
...
/** Game object that represents the spaceship.
 * @constructor */
Ship = function () {
  /**
   * Initialization of the shape and name of the spaceship
   */
  this.init("ship",
  ...
  /**
   * Initialization of the exhaust.
   */
  this.children.exhaust = new Sprite();
  /**
   * Initialization of the shape and name of the exhaust
   */ 
  ...
  /** The maximum number of bullets that can be displayed on the screnn at the same time.
   * @type {Integer} */                           
  this.bulletCounter = 0;

  this.postMove = this.wrapPostMove;
  /**
   * An array that consists of sprites that will collide with the spaceship.
   */                           
  this.collidesWith = ["asteroid", "bigalien", "alienbullet"];

  /**
   * Describes the behavior of the spaceship every frame and the interactions between the spacehsip and the input from keyboard
   * @param {Float} delta Time peroid of each frame
   */
      ...
      if (this.bulletCounter <= 0) {
        ...
        for (var i = 0; i < this.bullets.length; i++) {
          if (!this.bullets[i].visible) {
            ...
            //change *10 to *15
            bullet.x = this.x + vectorx * 15;
            bullet.y = this.y + vectory * 15 ;
  ...
  /**
   * Describes the behaviour of the spacehsip when it collides with other game obejcts.
   * @param {Sprite} other Game objects that can collide with the spaceship.
   */
...
/** Game object that represents the bullet.
 * @constructor */
Bullet = function () {
  /**
  * Initialization of the shape and name of the bullet object.
  */
  ...
  this.postMove = this.wrapPostMove;
  /**
  * Displays the transform of the bullet object.
  */
  this.configureTransform = function () {};
  /**
   * Draws the bullet object onto the screen
   */
  this.draw = function () {
    if (this.visible) {
      this.context.save();
...
Text = {
  ...
  renderText: function(text, size, x, y) {
    ...
    this.context.fillStyle = "white";//chagne the color(defualt:black)
    this.context.fill(); 
    //this.context.stroke();
    this.context.restore();
  },
...
Game = {
  ...
  /**
   * The maximum number of lives that the player's spaceship have during the game
   * @type {Integer}
   */ 
  lives: 0,
  canvasWidth: 800,
 ...
 /**
 * Gets the scores
 * @returns The scores that player attains.
 */
  getScore: function(){
    return Game.score;
  },
  ...
  FSM: {
    ...
    start: function () {
     ...
     /**
     * Game state where the spaceship is spwaned
     */
    spawn_ship: function () {
      Game.ship.x = Game.canvasWidth / 2+100;
    ...
    new_level: function () {
     ...
     /**
     * Game state where the health of the spaceship is 0
     */
    player_died: function () {
      ...
      //     document.getElementById("carsDiv").innerHTML = 1+ "<br />"+2;
      //     }
      // };
      } else {
        if (this.timer == null) {
    ...
    end_game: function () {
      ...
      Text.renderText('GAME OVER', 50, Game.canvasWidth/2 - 160, Game.canvasHeight/2 + 10);
            var final_score ='Your Score: ' + Game.score;
      Text.renderText(final_score, 30, 475, 375);
      if (this.timer == null) {
        this.timer = Date.now();
      }
...
$(function () {
   ...
   */
  var ship = new Ship();
  //ship's position at the beginning of the game
  ...
  /**
   * Creates the icons that represents the number lives the spaceship has.
   */
 ...
 */
  var mainLoop = function () {
    ...
    for (i = 0; i < Game.lives; i++) {
      context.save();
      extraDude.x = Game.canvasWidth - (15 * (i + 1));
      extraDude.y = 32;
